...
export const get404: RequestHandler = (_req, _res, next) => {
    return errorHandler(next, { statusCode: 404, message: "The route doesn't exist!" });
}

export const get500: ErrorRequestHandler = (error: ResponseError, _req, res, _next) => {
    console.error(error);
    const { statusCode, message } = error;
    if (isProduction && statusCode == undefined) {
